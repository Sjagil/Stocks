#!/usr/bin/env python3
"""
Strategy Combo Research Lab v1.0
================================
...
What it does
------------
1. Reads local daily OHLCV data from Parquet/CSV (no broker calls or secret-file reads).
...
PROGRAM_SCHEMA = "strategy_combo_research_lab_v1"
...
def build_ema_stretch(frame: pd.DataFrame, cache: FeatureCache, p: Mapping[str, Any]) -> TradeBatch:
    ...
    if p["mode"] == "percent":
        stretch = (ema - close) / ema
        entry = stretch > float(p["threshold"])
        score = stretch
    else:
